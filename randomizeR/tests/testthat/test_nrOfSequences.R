...
test_that("Output has correct number of sequences (with seed)", {
  ...
  Nr_Strata <- sample(seq(2, 6), 1)                    # Sample number of strata to consider
  N      <- sample(seq(2, 50, 2), Nr_Strata)           # Sample number of patients
  r      <- sample(30, 1)                              # Sample number of randomization sequences
  mti    <- sapply(N, function(x){ sample(x/2, 1)})    # Sample maximum tolerated imbalance
  p      <- sample(seq(0.5001, 1, 0.05), 1)            # biased coin parameter
  seed   <- sample(.Machine$integer.max, 1)            # Sample seed
  nr     <- sample(10,1)                               # sample number of blocks 
  blocks <- sample(seq(2, 20, 2), nr)                  # sample blocks
  gamma  <- sample(50, Nr_Strata)                      # Sample parameter for bbcd
  a      <- sample(50, Nr_Strata)                      # Sample parameter for abcd
  rho    <- sample(50, Nr_Strata)                      # Sample parameter for gbcd
  ...
  # 3. Test for Permuted Block Randomization
  # note that N is not needed here
  N_pbr <- sample(c(4, 8,12,16,20), Nr_Strata)
  size <- sample (c(2,4), 1)
  output1 <- genSeq(pbrPar(N = N_pbr, size = size ), r = r, seed = seed)
  expect_equal(nrow(getRandList(output1)), r)
  ...
  # 8. Test for Urn Design
  ini <- sample(seq(2, 20, 2), Nr_Strata)   # Sample initial urn composition
  add <- sample(seq(2, 20, 2), Nr_Strata)   # Sample number of balls that are added to urn each step
  output1 <- genSeq(udPar(N = N, ini = ini, add = add), r = r, seed = seed)
  expect_equal(nrow(getRandList(output1)), r)
