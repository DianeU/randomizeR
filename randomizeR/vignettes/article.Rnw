...
The current version of \pkg{randomizeR} is based on R 3.2.4. It can be loaded in an R 
session via:
<<loading>>=
library("randomizeR")
@
...
assumtion of normally distributed responses according to \eqref{eq:model}. 
